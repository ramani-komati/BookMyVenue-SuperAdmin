        ...
        {audit.map((a, i) => (
          <div key={i} className="card" style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontWeight: 700, color: 'var(--text-heading)', fontSize: 15.5 }}>{a.admin}</span>
              <span style={{ flex: 1 }} />
              <span style={{ fontSize: 13.5, color: 'var(--text-muted)' }}>{a.time}</span>
            </div>
            <div style={{ fontSize: 15.5 }}>{a.action} — <span style={{ fontWeight: 700, color: 'var(--text-heading)' }}>{a.target}</span></div>
